...
// third-party hosts whose images are not brand assets (map tiles, analytics pixels)
const SKIP_HOSTS =
  /(?:^|\.)(?:maps\.googleapis\.com|maps\.gstatic\.com|gstatic\.com|googletagmanager\.com|google-analytics\.com|doubleclick\.net|facebook\.net|fonts\.googleapis\.com)$/i;
...
const PAGES = argVal(
  "--pages",
  "/,/corporate,/news,/partnership,/merchandise,/membership",
)
  .split(",")
  .map((p) => p.trim())
  .filter(Boolean);
...
if (proxyServer && !BASE.includes("127.0.0.1") && !BASE.includes("localhost")) {
  ...
  // the TLS-inspecting egress proxy resets Chromium's TLS 1.3 handshake; cap at 1.2
...
context.on("response", async (response) => {
  try {
    ...
    if (ct.startsWith("image/")) {
      ...
      (ct.includes("font") || /\.(woff2?|ttf|otf)(\?|$)/i.test(url)) &&
      !isSkippedHost(url) &&
      response.ok()
    ) {
      const buffer = await response.body().catch(() => null);
      if (buffer && buffer.length > 0) {
        ...
        if (!existsSync(dest)) {
          ...
          log("  saved font", name, `(${(buffer.length / 1024).toFixed(1)}KB)`);
        }
      }
    }
  } catch {}
...
const collectComputedStyles = () => {
  ...
  for (const b of document.querySelectorAll(
    'button, [role="button"], .btn, [class*="button" i]',
  )) {
    sel.push([
      "button:" + (b.className || b.tagName).toString().slice(0, 60),
      b,
    ]);
  ...
  try {
    ...
    for (const u of domUrls) {
      ...
      if (!discovered.has(resolved))
        discovered.set(resolved, { firstSeenOn: url, via: "dom" });
...
for (const [u, meta] of imageResponses) {
  if (!discovered.has(u))
    discovered.set(u, { firstSeenOn: meta.firstSeenOn, via: "response" });
}

// fetch @font-face fonts declared in CSS that the pages never triggered
for (const [cssUrl, text] of cssFiles) {
  if (isSkippedHost(cssUrl)) continue;
  for (const m of text.matchAll(
    /url\(["']?([^"')]+\.(?:woff2?|ttf|otf))(?:\?[^"')]*)?["']?\)/gi,
  )) {
    try {
      const fontUrl = new URL(m[1], cssUrl).href;
      if (isSkippedHost(fontUrl)) continue;
      const name = filenameFromUrl(fontUrl);
      const dest = join(FONT_DIR, name);
      if (existsSync(dest)) continue;
      const resp = await page.request.get(fontUrl, { timeout: 30000 });
      if (resp.ok()) {
        const buf = await resp.body();
        writeFileSync(dest, buf);
        log("  saved font", name, `(${(buf.length / 1024).toFixed(1)}KB)`);
      }
    } catch {}
  }
}
...
for (const [url, meta] of discovered) {
  if (isSkippedHost(url)) {
    log("  skip (third-party host)", url);
    continue;
  }
...
for (const pd of pageData) {
  ...
  for (const sc of pd.sheetColors || [])
    tally(sc, `stylesheet rule (${pd.url})`);
}
for (const [cssUrl, text] of cssFiles) {
  for (const m of text.matchAll(COLOR_RE))
    tally(m[0], `raw css ${cssUrl.split("/").pop()}`);
}
for (const entry of manifest) {
  for (const f of entry.svgFills || [])
    tally(f, `SVG fill in ${entry.filename}`);
}
...
writeFileSync(
  join(BRAND_DIR, "extraction-raw.json"),
...
log(
  "wrote assets/brand/extraction-raw.json and assets/brand/image-manifest.json",
);
